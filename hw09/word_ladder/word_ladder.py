from queue import Queue
from stack import Stack


class WordLadder:
    """A class providing functionality to create word ladders"""
    def __init__(self, w1, w2, wordlist):
        """Initializing the WordLadder"""
        self.w1 = w1
        self.w2 = w2
        self.wordlist = wordlist
        while len(w1) != len(w2):
            print("Please enter two words with the same length")
            return

    def make_ladder(self):
        """Make a word ladder"""
        stack = Stack()
        queue = Queue()
        word_set = set()
        alpha = "abcdefghijklmnopqrstuvwxyz"
        new_word = ""

        stack.push(self.w1)  # push w1 into the stack
        queue.enqueue(stack)  # insert stack into queue
        temp = queue.dequeue()  # dequeue the stack
        while temp:
            word = temp.peek()
            for i in range(len(self.w1)):
                for j in alpha:
                    new_word = word[:i] + j + word[i+1:]  # error: word[i+1:-1]
                    if new_word in self.wordlist and new_word not in word_set:
                        word_set.add(new_word)
                        new_stack = temp.copy()  # create a new stack
                        new_stack.push(new_word)  # record all the temp answers
                        if new_word == self.w2:
                            return new_stack
                        else:
                            queue.enqueue(new_stack)

            if queue.isEmpty():
                return None
            else:
                temp = queue.dequeue()
